...
# Disable parallelism to avoid CUDA out of memory error
os.environ["TOKENIZERS_PARALLELISM"] = "false"


# https://huggingface.co/mixedbread-ai/mxbai-embed-large-v1
class MixBreadEmbeddings:
    ...
    async def _get_len_safe_embeddings(self, texts: str) -> List[List[float]]:
        """
        ...
        for i in _iter:
            ...
            embeddings = await self.pooling(outputs.last_hidden_state, inputs)
    ...
    async def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Embed search docs."""
        return await self._get_len_safe_embeddings(texts)

    async def embed_query(self, text: str) -> List[float]:
        """Embed query text."""
        response = await self.embed_documents([self.transform_query(text)])
        return response[0]
